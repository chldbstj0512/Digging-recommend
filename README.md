...
본 서비스의 ML process는 다음과 같습니다.</br>

## 개발 프로세스</br>
### 1. 데이터 수집
1만 곡에 가까운 데이터셋을 국내 스트리밍 사이트 crawling으로 직접 구축하였으며, 국내곡부터 해외가수까지 차트 기반 스크랩.</br>
데이터셋의 정보로는 아티스트에 관한 데이터셋과 곡에 관한 데이터셋이 존재하며, 아티스트는 장르, 활동년대, 성별, 그룹, 국가 등의 정보 포함</br>
곡 정보 데이터셋은 아티스트 데이터셋 기반 아티스트명, 앨범명, 곡명, 좋아요 수가 존재한다.</br>
song_data & artist_data 따로 수집하여 csv로 관리</br>
...
위 두 방식을 사용하여 딥러닝이나 ML 모델을 활용하여 패턴을 학습할 수 있음</br>

### 4. 모델 평가 및 최적화
개발팀 내 A/B 테스트를 통해 실제 사용자 환경에서의 추천 성능을 비교 및 평가할 수 있음</br>
